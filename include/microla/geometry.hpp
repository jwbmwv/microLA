...
#ifndef _MICROLA_GEOMETRY_HPP_
#define _MICROLA_GEOMETRY_HPP_
...
struct Ray
{
    ...
    MICROLA_NODISCARD Vec<T, 3> at(T t) const { return origin + direction * t; }
};
...
struct Plane
{
    ...
    MICROLA_NODISCARD static Plane from_point_normal(const Vec<T, 3>& point, const Vec<T, 3>& normal)
    {
    ...
    /// @return Plane instance.
    MICROLA_NODISCARD static Plane from_points(const Vec<T, 3>& p0, const Vec<T, 3>& p1, const Vec<T, 3>& p2)
    {
        Vec<T, 3> v1 = p1 - p0;
        Vec<T, 3> v2 = p2 - p0;
    ...
    MICROLA_NODISCARD T distance(const Vec<T, 3>& point) const { return normal.dot(point) + d; }

    /// @brief Signed distance from point to plane.
    MICROLA_NODISCARD T signed_distance(const Vec<T, 3>& point) const { return distance(point); }

    /// @brief Project a point onto the plane.
    /// @param point Point to project.
    /// @return Projected point on the plane.
    MICROLA_NODISCARD Vec<T, 3> project(const Vec<T, 3>& point) const { return point - normal * distance(point); }

    /// @brief Ray-plane intersection.
    /// @param ray Ray to test.
    /// @return Distance along ray to intersection point, or std::nullopt if no intersection.
    MICROLA_NODISCARD std::optional<T> intersect(const Ray<T>& ray) const
    {
...
struct AABB
{
    ...
    MICROLA_NODISCARD static AABB from_center_extents(const Vec<T, 3>& center, const Vec<T, 3>& extents)
    {
    ...
    MICROLA_NODISCARD Vec<T, 3> center() const { return (min + max) * static_cast<T>(0.5); }

    /// @brief Get half-extents (distance from center to faces).
    /// @return Half-extents along each axis.
    MICROLA_NODISCARD Vec<T, 3> extents() const { return (max - min) * static_cast<T>(0.5); }

    /// @brief Get size (full width, height, depth).
    /// @return Size along each axis.
    MICROLA_NODISCARD Vec<T, 3> size() const { return max - min; }

    /// @brief Check if a point is inside the AABB.
    /// @param point Point to test.
    /// @return True if point is inside or on the boundary.
    MICROLA_NODISCARD bool contains(const Vec<T, 3>& point) const
    {
    ...
    MICROLA_NODISCARD bool intersects(const AABB& other) const
    {
    ...
    MICROLA_NODISCARD AABB merge(const AABB& other) const
    {
    ...
    MICROLA_NODISCARD AABB expand(T amount) const
    {
    ...
    MICROLA_NODISCARD bool contains(const AABB& other) const
    {
    ...
    MICROLA_NODISCARD std::optional<std::pair<T, T>> intersect(const Ray<T>& ray) const
    {
    ...
    /// @return Surface area.
    MICROLA_NODISCARD T surface_area() const
    {
        Vec<T, 3> s = size();
...
struct Sphere
{
    ...
    MICROLA_NODISCARD bool intersects(const Sphere& other) const
    {
    ...
    MICROLA_NODISCARD AABB<T> to_aabb() const
    {
    ...
    /// @return Surface area.
    MICROLA_NODISCARD T surface_area() const { return static_cast<T>(4) * constants::pi<T>() * radius * radius; }

    /// @brief Get volume.
    /// @return Volume.
    MICROLA_NODISCARD T volume() const
    {
    ...
    MICROLA_NODISCARD bool contains(const Sphere& other) const
    {
        Vec<T, 3> diff = center - other.center;
    ...
    /// @return True if they intersect.
    MICROLA_NODISCARD bool intersects(const AABB<T>& aabb) const
    {
...
struct Triangle
{
    ...
    MICROLA_NODISCARD Vec<T, 3> normal() const
    {
        Vec<T, 3> e1 = v1 - v0;
        Vec<T, 3> e2 = v2 - v0;
    ...
    MICROLA_NODISCARD T area() const
    {
        Vec<T, 3> e1 = v1 - v0;
        Vec<T, 3> e2 = v2 - v0;
    ...
    MICROLA_NODISCARD Vec<T, 3> centroid() const { return (v0 + v1 + v2) * (static_cast<T>(1) / static_cast<T>(3)); }
...
struct Frustum
{
    ...
    MICROLA_NODISCARD static Frustum from_matrix(const Mat<T, 4, 4>& vp)
    {
        Frustum f;
    ...
    MICROLA_NODISCARD bool contains(const Sphere<T>& sphere) const
    {
        for (const auto& plane : planes)
        {
    ...
    MICROLA_NODISCARD bool intersects(const Sphere<T>& sphere) const
    {
        for (const auto& plane : planes)
        {
            if (plane.distance(sphere.center) < -sphere.radius)
