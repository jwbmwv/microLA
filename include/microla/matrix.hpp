...
#ifndef _MICROLA_MICROLA_HPP_
#define _MICROLA_MICROLA_HPP_
...
// ============================================================================
// Identity Matrix Helper for C++11 Compatibility
// ============================================================================
// C++11 doesn't allow loops in constexpr functions, so we use template
// specialization to provide loop-free implementations for common sizes.

/// @brief Helper struct for creating identity matrices (forward declaration)
template<typename T, std::uint32_t R, std::uint32_t C, typename Enable>
...
template<typename T = float, std::uint32_t R = 4, std::uint32_t C = 4>
class Mat
{
public:
    ...
    /// @details Creates a matrix with all elements initialized to the default value T(0).
    ///          This constructor is constexpr, allowing compile-time initialization.
    MICROLA_CONSTEXPR Mat() noexcept : data{}
    {
    ...
    MICROLA_CONSTEXPR Mat(std::initializer_list<T> init)
    {
    ...
    ///          allowing compile-time generation of identity matrices. For C++11, uses
    ///          specialized implementations for 2×2, 3×3, and 4×4 matrices that work
    ///          in constexpr contexts. For C++14+, all sizes work in constexpr contexts.
    /// @note Static assertion will fail if called on non-square matrix types
    static constexpr Mat identity() { return IdentityHelper<T, R, C, void>::create(); }
    ...
    ///          This is a constexpr function for compile-time initialization (C++14+).
    static MICROLA_CONSTEXPR14 Mat zero()
    {
        Mat result;
        for (std::uint32_t i = 0; i < R * C; ++i)
        {
    ...
    ///          This is a constexpr function for compile-time initialization (C++14+).
    static MICROLA_CONSTEXPR14 Mat ones()
    {
        Mat result;
        for (std::uint32_t i = 0; i < R * C; ++i)
        {
    ...
    static Mat diagonal(const Vec<T, N>& diag)
    {
    ...
    MICROLA_NODISCARD T* operator[](std::uint32_t row) noexcept { return &data[row * C]; }
    ...
    MICROLA_NODISCARD const T* operator[](std::uint32_t row) const noexcept { return &data[row * C]; }
    ...
    /// @details Allows accessing matrix elements using m(i, j) syntax
    MICROLA_NODISCARD MICROLA_CONSTEXPR T& operator()(std::uint32_t row, std::uint32_t col) noexcept
    {
    ...
    /// @details Allows accessing matrix elements using m(i, j) syntax on const matrices
    MICROLA_NODISCARD MICROLA_CONSTEXPR const T& operator()(std::uint32_t row, std::uint32_t col) const noexcept
    {
        return data[row * C + col];
    }
    ...
    template<std::uint32_t BR, std::uint32_t BC>
    MICROLA_NODISCARD Mat<T, BR, BC> block(std::uint32_t start_row, std::uint32_t start_col) const
    {
    ...
    /// @note Compile-time assertion ensures the block fits within matrix bounds
    template<std::uint32_t BR, std::uint32_t BC>
    void set_block(std::uint32_t start_row, std::uint32_t start_col, const Mat<T, BR, BC>& block_data)
    {
        assert(start_row + BR <= R && "Block exceeds matrix row bounds");
        assert(start_col + BC <= C && "Block exceeds matrix column bounds");
    ...
    MICROLA_NODISCARD Mat<T, 1, C> row(std::uint32_t row_idx) const
    {
    ...
    MICROLA_NODISCARD Mat<T, R, 1> col(std::uint32_t col_idx) const
    {
    ...
    void set_row(std::uint32_t row_idx, const Mat<T, 1, C>& row_data)
    {
        assert(row_idx < R && "Row index out of bounds");
        for (std::uint32_t j = 0; j < C; ++j)
        {
    ...
    void set_col(std::uint32_t col_idx, const Mat<T, R, 1>& col_data)
    {
        assert(col_idx < C && "Column index out of bounds");
        for (std::uint32_t i = 0; i < R; ++i)
        {
    ...
    MICROLA_NODISCARD T determinant() const
    {
        static_assert(R == C, "Determinant is only defined for square matrices.");

        MICROLA_IF_CONSTEXPR(R == 1)
        {
            return data[0];
        }
        else MICROLA_IF_CONSTEXPR(R == 2)
        {
            return data[0] * data[3] - data[1] * data[2];
        }
        else MICROLA_IF_CONSTEXPR(R == 3)
        {
        ...
        else
        {
            ...
            {
                // Find pivot
                std::uint32_t pivot = i;
                T maxVal = std::abs(temp(i, i));
                for (std::uint32_t k = i + 1; k < R; ++k)
                {
                    T absVal = std::abs(temp(k, i));
                    if (absVal > maxVal)
                    {
                        maxVal = absVal;
                        pivot = k;
    ...
    MICROLA_NODISCARD Mat inverse() const
    {
        ...
        else MICROLA_IF_CONSTEXPR(R == 3)
        {
            ...
            result(0, 0) = (data[4] * data[8] - data[5] * data[7]) / det;
            result(0, 1) = (data[2] * data[7] - data[1] * data[8]) / det;
            result(0, 2) = (data[1] * data[5] - data[2] * data[4]) / det;
            result(1, 0) = (data[5] * data[6] - data[3] * data[8]) / det;
            result(1, 1) = (data[0] * data[8] - data[2] * data[6]) / det;
            result(1, 2) = (data[2] * data[3] - data[0] * data[5]) / det;
            result(2, 0) = (data[3] * data[7] - data[4] * data[6]) / det;
            result(2, 1) = (data[1] * data[6] - data[0] * data[7]) / det;
            result(2, 2) = (data[0] * data[4] - data[1] * data[3]) / det;
            return result;
        }
        else
        {
            ...
            for (std::uint32_t i = 0; i < R; ++i)
            {
                ...
                for (std::uint32_t k = i + 1; k < R; ++k)
                {
                    T absVal = std::abs(augmented[k][i]);
                    if (absVal > maxVal)
                    {
                        maxVal = absVal;
                        pivot = k;
    ...
    MICROLA_NODISCARD Mat<T, C, R> pseudoinverse(T tolerance = 1e-9) const
    {
        ...
        for (std::uint32_t i = 0; i < std::min(R, C); ++i)
        {
            T sigma = S(i, i);
        ...
        for (std::uint32_t i = 0; i < C; ++i)
        {
            for (std::uint32_t j = 0; j < R; ++j)
            {
                ...
                for (std::uint32_t k = 0; k < std::min(C, R); ++k)
                {
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR bool operator==(const Mat& other) const noexcept
    {
        if (this == &other)
            return true;
        for (std::uint32_t i = 0; i < R * C; ++i)
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR bool operator!=(const Mat& other) const noexcept { return !(*this == other); }
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR Mat operator+(const Mat& other) const noexcept
    {
#ifdef CONFIG_MICROLA_CMSIS
        if (std::is_same<T, float>::value)
        {
            ...
            arm_mat_init_f32(&src1, R, C, const_cast<float*>(reinterpret_cast<const float*>(data)));
            arm_mat_init_f32(&src2, R, C, const_cast<float*>(reinterpret_cast<const float*>(other.data)));
            arm_mat_init_f32(&dst, R, C, reinterpret_cast<float*>(result.data));
    ...
    template<std::uint32_t OtherC>
    MICROLA_NODISCARD Mat<T, R, OtherC> operator*(const Mat<T, C, OtherC>& other) const
    {
        Mat<T, R, OtherC> result;

#ifdef CONFIG_MICROLA_RISCV
        // SIMD optimization for 4x4 float matrices using RISC-V V extension
        if (std::is_same<T, float>::value && R == 4 && C == 4 && OtherC == 4)
        {
            const float* a = reinterpret_cast<const float*>(data);
            const float* b = reinterpret_cast<const float*>(other.data);
            float* r = reinterpret_cast<float*>(result.data);
        ...
        if (std::is_same<T, float>::value)
        {
            arm_matrix_instance_f32 src1, src2, dst;
            arm_mat_init_f32(&src1, R, C, const_cast<float*>(reinterpret_cast<const float*>(data)));
            arm_mat_init_f32(&src2, C, OtherC, const_cast<float*>(reinterpret_cast<const float*>(other.data)));
            arm_mat_init_f32(&dst, R, OtherC, reinterpret_cast<float*>(result.data));
        ...
        for (std::uint32_t i = 0; i < R; ++i)
        {
            for (std::uint32_t j = 0; j < OtherC; ++j)
            {
    ...
    MICROLA_NODISCARD Vec<T, R> operator*(const Vec<T, VecSize>& vec) const
    {
        ...
        if (std::is_same<T, float>::value && R == 4 && C == 4)
        {
        ...
        else if (std::is_same<T, float>::value && R == 3 && C == 3)
        {
            const float* m = reinterpret_cast<const float*>(data);
            const float* v = reinterpret_cast<const float*>(vec.data);
            float* r = reinterpret_cast<float*>(result.data);
        ...
        if (std::is_same<T, float>::value)
        {
            ...
            arm_mat_init_f32(&mat, R, C, const_cast<float*>(reinterpret_cast<const float*>(data)));
            arm_mat_init_f32(&vec_mat, C, 1, const_cast<float*>(reinterpret_cast<const float*>(vec.data)));
            arm_mat_init_f32(&result_mat, R, 1, reinterpret_cast<float*>(result.data));
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR Mat operator*(T scalar) const noexcept
    {
#ifdef CONFIG_MICROLA_CMSIS
        if (std::is_same<T, float>::value)
        {
            ...
            arm_mat_init_f32(&src, R, C, const_cast<float*>(reinterpret_cast<const float*>(data)));
            arm_mat_init_f32(&dst, R, C, reinterpret_cast<float*>(result.data));
    ...
    MICROLA_CONSTEXPR Mat& operator+=(const Mat& other) noexcept
    {
        for (std::uint32_t i = 0; i < R * C; ++i)
        {
            data[i] += other.data[i];
    ...
    MICROLA_CONSTEXPR Mat& operator*=(T scalar) noexcept
    {
        for (std::uint32_t i = 0; i < R * C; ++i)
        {
            data[i] *= scalar;
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR Mat<T, C, R> transpose() const noexcept
    {
#ifdef CONFIG_MICROLA_CMSIS
        if (std::is_same<T, float>::value)
        {
            ...
            arm_matrix_instance_f32 src, dst;
            arm_mat_init_f32(&src, R, C, const_cast<float*>(reinterpret_cast<const float*>(data)));
            arm_mat_init_f32(&dst, C, R, reinterpret_cast<float*>(result.data));
    ...
    static constexpr std::uint32_t size() noexcept { return R * C; }

    /// @brief Returns the number of rows in the matrix
    /// @return The number of rows (R)
    static constexpr std::uint32_t rows() noexcept { return R; }

    /// @brief Returns the number of columns in the matrix
    /// @return The number of columns (C)
    static constexpr std::uint32_t cols() noexcept { return C; }
    ...
    MICROLA_NODISCARD static MICROLA_CONSTEXPR26 Mat rotation_x(T angle)
    {
    ...
    ///          In C++26+, this function is constexpr when angle is known at compile-time.
    /// @note Requires R == 3 and C == 3
    MICROLA_NODISCARD static MICROLA_CONSTEXPR26 Mat rotation_y(T angle)
    {
    ...
    ///          In C++26+, this function is constexpr when angle is known at compile-time.
    /// @note Requires R == 3 and C == 3
    MICROLA_NODISCARD static MICROLA_CONSTEXPR26 Mat rotation_z(T angle)
    {
    ...
    ///          In C++26+, this function is constexpr when angle is known at compile-time.
    MICROLA_NODISCARD static MICROLA_CONSTEXPR26 Mat<T, 2, 2> rotation(T angle)
    {
    ...
    MICROLA_NODISCARD static MICROLA_CONSTEXPR26 Mat<T, 3, 3> rotation_axis_angle(const Vec<T, 3>& axis, T angle)
    {
    ...
    ///          In C++26+, this function is constexpr when parameters are known at compile-time.
    MICROLA_NODISCARD static MICROLA_CONSTEXPR26 Mat<T, 3, 3> look_at(const Vec<T, 3>& target, const Vec<T, 3>& up)
    {
    ...
    ///          Only available for square matrices (R == C).
    MICROLA_NODISCARD MICROLA_CONSTEXPR T trace() const noexcept
    {
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR26 T frobenius_norm() const noexcept
    {
        T sum = static_cast<T>(0);
        for (std::uint32_t i = 0; i < R * C; ++i)
        {
    ...
    MICROLA_NODISCARD T norm_inf() const noexcept
    {
    ...
    MICROLA_NODISCARD T norm_1() const noexcept
    {
        T max_sum = static_cast<T>(0);
        for (std::uint32_t j = 0; j < C; ++j)
        {
    ...
    /// @return The infinity norm ||A||_∞
    MICROLA_NODISCARD T infinity_norm() const noexcept { return norm_inf(); }

    /// @brief Alias for norm_1() - computes the 1-norm
    /// @return The 1-norm ||A||_1
    MICROLA_NODISCARD T one_norm() const noexcept { return norm_1(); }
    ...
    MICROLA_NODISCARD T condition_number() const
    {
    ...
    MICROLA_NODISCARD static MICROLA_CONSTEXPR Mat scale(const Mat& m, T scalar) noexcept
    {
        Mat result;
        for (std::uint32_t i = 0; i < R * C; ++i)
        {
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR26 Vec<T, 3> euler_angles() const
    {
    ...
    ///          Only available for square matrices (R == C).
    MICROLA_NODISCARD std::tuple<Mat, Mat, Mat> lu() const
    {
    ...
    ///          Only available for square matrices (R == C).
    /// @throws std::runtime_error if matrix is not positive-definite (when exceptions enabled)
    /// @note Matrix must be symmetric and positive-definite for this decomposition to succeed.
    ///       For embedded systems without exceptions, check return value validity.
    MICROLA_NODISCARD Mat cholesky() const
    {
        ...
        for (std::uint32_t i = 0; i < R; ++i)
        {
            for (std::uint32_t j = 0; j <= i; ++j)
            {
                ...
                if (i == j)
                {
                    ...
                    {
#ifdef MICROLA_HAS_EXCEPTIONS
                        throw std::runtime_error("Matrix is not positive-definite");
#else
                        // For embedded systems without exceptions, set to small epsilon
                        // Caller should verify result validity
                        diag_val = std::numeric_limits<T>::epsilon();
#endif
                    }
    ...
    MICROLA_NODISCARD std::vector<T> eigenvaluesQR(std::uint32_t maxIterations = 100, T tolerance = 1e-6) const
    {
        static_assert(R == C, "Eigenvalue computation is only defined for square matrices.");
        Mat A = *this;
        Mat Q, R_mat;  // Renamed R to R_mat to avoid shadowing the template parameter
    ...
    ///          Only available for square matrices (R == C).
    MICROLA_NODISCARD std::tuple<Mat, Mat> qr() const
    {
        static_assert(R == C, "QR decomposition is only defined for square matrices.");
        Mat Q = identity();
        Mat R_mat = *this;

        for (std::uint32_t k = 0; k < std::min(R - 1, C); ++k)
        {
            // Compute column norm from k to end
            T norm = T(0);
            for (std::uint32_t i = k; i < R; ++i)
            {
                                                                               ...
                                                                               T tolerance = 1e-9) const
    {
        ...
        MICROLA_IF_CONSTEXPR(R >= C)
        {
            ...
            for (std::uint32_t iter = 0; iter < maxIterations; ++iter)
            {
                T maxOffDiag = static_cast<T>(0);
                std::uint32_t p = 0, q = 1;

                // Find largest off-diagonal element
                for (std::uint32_t i = 0; i < C; ++i)
                {
                    for (std::uint32_t j = i + 1; j < C; ++j)
                    {
        ...
        {
            // Wide matrix: compute U from A * A^T (similar approach)
            Mat<T, R, R> AAT;
            for (std::uint32_t i = 0; i < R; ++i)
            {
                for (std::uint32_t j = 0; j < R; ++j)
            ...
            // Similar Jacobi iterations for AAT...
    ...
    MICROLA_NODISCARD MICROLA_CONSTEXPR std::uint32_t rank() const
    {
        Mat temp = *this;
        std::uint32_t rank = 0;
        ...
        for (std::uint32_t row = 0; row < R; ++row)
        {
            ...
            for (std::uint32_t col = 0; col < C; ++col)
            {
                if (temp(row, col) != T(0))
                {
                    nonZeroRow = true;
            ...
            {
                ++rank;
                for (std::uint32_t i = row + 1; i < R; ++i)
                {
                    T factor = temp(i, row) / temp(row, row);
                    for (std::uint32_t j = row; j < C; ++j)
                    {
...
template<typename T, std::uint32_t R, std::uint32_t C, typename Enable = void>
struct IdentityHelper
{
    static MICROLA_CONSTEXPR Mat<T, R, C> create()
    {
...
// C++11 specializations using direct initialization (no loops)

/// @brief C++11-compatible 2×2 identity matrix specialization
template<typename T>
struct IdentityHelper<T, 2, 2, void>
{
    static constexpr Mat<T, 2, 2> create() noexcept { return Mat<T, 2, 2>{T(1), T(0), T(0), T(1)}; }
};

/// @brief C++11-compatible 3×3 identity matrix specialization
template<typename T>
struct IdentityHelper<T, 3, 3, void>
{
    static constexpr Mat<T, 3, 3> create() noexcept
    {
        return Mat<T, 3, 3>{T(1), T(0), T(0), T(0), T(1), T(0), T(0), T(0), T(1)};
    }
};

/// @brief C++11-compatible 4×4 identity matrix specialization
template<typename T>
struct IdentityHelper<T, 4, 4, void>
{
    static constexpr Mat<T, 4, 4> create() noexcept
    {
        return Mat<T, 4, 4>{T(1), T(0), T(0), T(0), T(0), T(1), T(0), T(0),
