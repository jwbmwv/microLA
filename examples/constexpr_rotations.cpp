...
int main()
{
    ...
    // ========================================
    std::cout << "--- C++14+: Special Angle Rotations ---\n";
    std::cout << "These compile to constants!\n\n";

#if __cplusplus >= 201402L
    // Identity matrix - true compile-time constant (C++14+)
...
#else
    // C++11: These are still optimized but not constexpr
    auto identity2 = Mat<float, 2, 2>::identity();
    auto identity3 = Mat<float, 3, 3>::identity();
    auto identity4 = Mat<float, 4, 4>::identity();
#endif
    ...
    // 2D rotation
    auto R2D = Mat<float, 2, 2>::rotation(deg_to_rad(90.0f));
    print_matrix("90° rotation in 2D", R2D);
