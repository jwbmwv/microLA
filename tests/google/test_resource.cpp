...
TEST(ResourceChecks, CInteropLayout)
{
    ...
    auto* v = reinterpret_cast<Vec<float, 3>*>(c_vec);
    ...
    auto* q = reinterpret_cast<Quaternion<float>*>(c_quat);
...
TEST(ResourceChecks, MemoryFootprint)
{
    ...
    std::size_t sensor_state = (sizeof(Vec<float, 3>)) * 3 + (sizeof(Quaternion<float>));
    ...
    std::size_t kalman_2d = (sizeof(Vec<float, 2>)) * 2 + (sizeof(Mat<float, 2, 2>)) * 2;
