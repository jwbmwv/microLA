...
int main()
{
    ...
    benchmark("Create Rotation Matrix (runtime)",
              [&]() { result_m4 = SquareMat<float, 4>::rotation_x(constants::pi<float> / 4.0f); });

    benchmark("Create Translation Matrix",
              [&]() { result_m4 = SquareMat<float, 4>::translation(Vec3f{1.0f, 2.0f, 3.0f}); });

    benchmark("Create Scale Matrix", [&]() { result_m4 = SquareMat<float, 4>::scaling(Vec3f{2.0f, 2.0f, 2.0f}); });
    ...
    benchmark("LU Decomposition", [&]() { auto lu = decomp_mat.lu_decompose(); }, 100000);

    benchmark("QR Decomposition", [&]() { auto qr = decomp_mat.qr_decompose(); }, 100000);

    benchmark("Cholesky Decomposition", [&]() { auto chol = decomp_mat.cholesky_decompose(); }, 100000);
